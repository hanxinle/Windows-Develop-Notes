...
BEGIN_MESSAGE_MAP(CCFileDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_WRITEFILE, &CCFileDlg::OnBnClickedWritefile)
	ON_BN_CLICKED(IDC_READFILE, &CCFileDlg::OnBnClickedReadfile)
END_MESSAGE_MAP()
...
void CCFileDlg::OnBnClickedWritefile()
{
...
void CCFileDlg::OnBnClickedReadfile()
{
	// TODO: 在此添加控件通知处理程序代码
	UpdateData(TRUE);   //get data in edit control
	m_strName.TrimLeft();
	...
	if (m_strName.GetLength() == 0) { MessageBox(_T("No file !")); }
		...
		char * szBuff;
		ULONGLONG length = myfile.GetLength();
